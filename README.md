...
The index.php file loads the Composer generated autoloader definition, and then retrieves an instance of the K application **Core** class. When **Core** class is instantiated optionally midlewares and hooks are added and then execute method is called to start generating response.
...
```php
$route->get('article/:id', "ArticleController", "getArticleAction"); 
```

Captured ID will be apended to **Request** object get array
...
Middleware is a anything that is callable and accepts another callabale as parametar (next middleware on stack):
...
Different frameworks use middleware differently. K implements middlewares as stack. Each new middleware will be put on top of existing middlewares. The structure expands as additional middleware layers are added. The last middleware layer added is the first to be executed. By default application core routing process will be automaticly added as middleware.
...
*   Exception catched (internal.error)
